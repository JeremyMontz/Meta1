...
| index | `tc-index.test.mjs` | #340 | 1 | `index.html` source carries each named homepage section: `window.PAGE_HOME` block + AgentGraph mount + live GitHub activity feed mount + portfolio-teaser / writing-list / about sections. Pass-1 structural presence (disjunctive/CI-safe); content snapshot deferred to pass 2 | authored |
